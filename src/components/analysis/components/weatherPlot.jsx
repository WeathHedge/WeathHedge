...
export function WeatherPlot({ state, setState, width, loadingState }) {
  ...
  return (
    <XYPlot
      ...
      onMouseLeave={() =>
        !state.keepCrosshair
          ? setState({ ...state, crosshairValues: [] })
          : null
      }
      style={
        loadingState
          ? { filter: "blur(3px)", transition: "filter .3s ease-in-out" }
          : { filter: "blur(0)", transition: "filter .3s ease-in-out" }
      }
      xDomain={
        lastDrawLocation && [lastDrawLocation.left, lastDrawLocation.right]
      }
    >
      {state.currentVisMode === null ? (
        <XAxis title="" attr="x" hideTicks />
      ) : (
        <XAxis title="" attr="x" />
      )}
        ...
        <LineSeries
          ...
          onNearestX={(value, { index }) =>
            !state.keepCrosshair
              ? setState({
                  ...state,
                  crosshairValues: state.tempData.map((d) => d[index]),
                })
              : null
          }
          data={state.tempData[0]}
        />
      ) : null}
      {state.showMax ? (
        <AreaSeries
      ...
      {state.showMedian ? (
        <LineSeries
          ...
          strokeStyle="solid"
          data={state.tempDataMedian}
        />
      ) : null}
      {state.showTrend && state.tempData[0] && state.showMax ? (
        <LineSeries
          strokeStyle="dashed"
          color={myColors.Red}
          data={leastSquaresLinearRegression(state.tempData[0])}
        />
      ) : null}
      {state.showTrend && state.tempData[1] && state.showMean ? (
        <LineSeries
          strokeStyle="dashed"
          color={myColors.Green}
          data={leastSquaresLinearRegression(state.tempData[1])}
        />
      ) : null}
      {state.showTrend && state.tempData[2] && state.showMin ? (
        <LineSeries
          strokeStyle="dashed"
          color={"white"}
          data={leastSquaresLinearRegression(state.tempData[2])}
        />
      ) : null}
      {state.showPrec ? (
        <LineSeries
          color={"lightblue"}
          curve={"curveStep"}
          data={state.tempData[3]}
        />
      ) : null}
      {state.showPrec ? (
        <LineSeries
          color={"black"}
          data={leastSquaresLinearRegression(state.tempData[3])}
        />
      ) : null}
        ...
        <Crosshair
          ...
          titleFormat={(d) => {
            if (state.currentVisMode === visualizationModes.MonthHistory) {
              return {
                title: "x",
                value: `${d[0].x.getFullYear()} ${
                  months[d[0].x.getMonth() + 1]
                }`,
              };
            } else if (
              state.currentVisMode === visualizationModes.WeekHistory
            ) {
      ...
      {state.crosshairValues.length === 0 ||
      state.currentVisMode === null ? null : (
        <MarkSeries data={fetchMarkerData()} color={"white"} stroke="gray " />
      )}
      {state.currentVisMode === null ? null : (
        <Highlight
          enableY={false}
      ...
      {loadingState ? <div className={styles.loadingCover}>...</div> : null}
    </XYPlot>
  );
}
