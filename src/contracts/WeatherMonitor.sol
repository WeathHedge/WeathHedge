...
contract FunctionsConsumerExample is FunctionsClient, ConfirmedOwner {
    ...
    constructor(
        address router,address _weatherDerivativeAddress 
    ) FunctionsClient(router) ConfirmedOwner(msg.sender) { weatherDerivativeContract = WeatherDerivative(_weatherDerivativeAddress);}
    ...
    function fulfillRequest(
        ...
        weatherDerivativeContract.monitorWeatherData(bytesToUint(s_lastResponse));
        s_lastError = err;
    ...
    function callMonitorWeatherData(uint256 temperature) external {
     ...
     function bytesToUint(bytes memory data) public pure returns (uint) {
        require(data.length >= 32, "Data must be at least 32 bytes long");
        
        uint result;
        assembly {
            // Convert the bytes to uint256 (32 bytes)
